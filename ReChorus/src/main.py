...
# def parse_global_args(parser):
#     # 添加全局配置参数
#     parser.add_argument('--gpu', type=str, default='0', help='指定CUDA_VISIBLE_DEVICES，默认为CPU')
#     parser.add_argument('--verbose', type=int, default=logging.INFO, help='日志等级，0, 10, ..., 50')
#     parser.add_argument('--log_file', type=str, default='', help='日志文件路径')
#     parser.add_argument('--random_seed', type=int, default=0, help='随机种子')
#     parser.add_argument('--load', type=int, default=0, help='是否加载已训练的模型继续训练')
#     parser.add_argument('--train', type=int, default=1, help='是否训练模型')
#     parser.add_argument('--save_final_results', type=int, default=1, help='是否保存最终的验证与测试结果')
#     parser.add_argument('--regenerate', type=int, default=0, help='是否重新生成中间文件')
#     return parser
...
#     corpus_path = os.path.join(args.path, args.dataset, model_name.reader + args.data_appendix + '.pkl')
#     if not args.regenerate and os.path.exists(corpus_path):
#         logging.info('加载语料库来自：{}'.format(corpus_path))
#         corpus = pickle.load(open(corpus_path, 'rb'))
#     else:
#         corpus = reader_name(args)
#         logging.info('保存语料库至：{}'.format(corpus_path))
#         pickle.dump(corpus, open(corpus_path, 'wb'))

#     # 初始化模型
#     model = model_name(args, corpus).to(args.device)
#     logging.info('#参数数量: {}'.format(model.count_variables()))
#     logging.info(model)

#     # 定义数据集
#     data_dict = {phase: model_name.Dataset(model, corpus, phase) for phase in ['train', 'dev', 'test']}
#     for phase in data_dict:
#         data_dict[phase].prepare()

#     # 执行模型
#     runner = runner_name(args)
#     logging.info('训练前测试结果: ' + runner.print_res(data_dict['test']))
#     if args.load > 0:
#         model.load_model()
#     if args.train > 0:
#         runner.train(data_dict)
...
#     # 评估最终结果
#     eval_res = runner.print_res(data_dict['dev'])
#     logging.info(os.linesep + '训练后开发集结果: ' + eval_res)
#     eval_res = runner.print_res(data_dict['test'])
#     logging.info(os.linesep + '训练后测试集结果: ' + eval_res)
...
#     if init_args.model_mode == 'CTR':  # CTR任务
#         logging.info('保存CTR预测结果至: {}'.format(result_path))
#         predictions, labels = runner.predict(dataset)
#         users, items = [], []
#         for i in range(len(dataset)):
#             info = dataset[i]
#             users.append(info['user_id'])
#             items.append(info['item_id'][0])
#         rec_df = pd.DataFrame({'user_id': users, 'item_id': items, 'pCTR': predictions, 'label': labels})
#         rec_df.to_csv(result_path, sep=args.sep, index=False)
#     elif init_args.model_mode in ['TopK', '']:  # TopK推荐任务
#         logging.info('保存前{}个推荐结果至: {}'.format(topk, result_path))
...
#             rec_items.append([x[0] for x in sorted_lst])
#             rec_predictions.append([x[1] for x in sorted_lst])
#         rec_df = pd.DataFrame({'user_id': users, 'rec_items': rec_items, 'rec_predictions': rec_predictions})
#         rec_df.to_csv(result_path, sep=args.sep, index=False)
#     elif init_args.model_mode in ['Impression', 'General', 'Sequential']:  # List-wise重新排序任务
#         logging.info('保存所有推荐结果至: {}'.format(result_path))
#         predictions = runner.predict(dataset)
#         users, pos_items, pos_predictions, neg_items, neg_predictions = [], [], [], [], []
#         for i in range(len(dataset)):
#             info = dataset[i]
#             users.append(info['user_id'])
#             pos_items.append(info['pos_items'])
...
#             pos_predictions.append(predictions[i][:dataset.pos_len])
#             neg_predictions.append(predictions[i][:dataset.neg_len])
#         rec_df = pd.DataFrame({'user_id': users, 'pos_items': pos_items, 'pos_predictions': pos_predictions,
#                                'neg_items': neg_items, 'neg_predictions': neg_predictions})
#         rec_df.to_csv(result_path, sep=args.sep, index=False)
#     else:
#         return 0
#     logging.info("{}预测结果已保存!".format(dataset.phase))
...
#     # 初始化解析器并获取模型名称和模式
#     init_parser = argparse.ArgumentParser(description='Model')
#     init_parser.add_argument('--model_name', type=str, default='SASRec', help='选择模型')
#     init_parser.add_argument('--model_mode', type=str, default='',
#                              help='模型模式（如“CTR”或“TopK”推荐任务等）')
#     init_args, init_extras = init_parser.parse_known_args()
